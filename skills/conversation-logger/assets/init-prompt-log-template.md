# {Project Name} - Prompt Log

**Date**: {YYYY-MM-DD}
**Status**: {Planning / In Progress / Complete} {emoji}
**Deliverable**: [{deliverable-name}](./{deliverable-path})

---

## Session 1: {Session Theme} ({Mon DD, YYYY})

### Prompt 1: {Brief Description} (9:15 AM)

> {Full user prompt text - capture verbatim, only abbreviate if exceeds ~500 words}

→ Response: {1 line what was decided/done}
→ Action: {1 line files/tools used}

---

### Prompt 2: {Brief Description} (9:32 AM)

> {Full user prompt}

→ Response: {1 line}
→ Action: {1 line} ✅

---

### Prompt 3: {Brief Description} (9:48 AM)

> {Full user prompt}

→ Response: {1 line}
→ Action: {1 line}

---

## Session 2: {Session Theme} ({Mon DD, YYYY})

### Prompt 4: {Brief Description} (10:05 AM)

> {Full user prompt}

→ Response: {1 line}
→ Action: {1 line}
